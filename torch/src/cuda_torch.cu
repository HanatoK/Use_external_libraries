...
CUDAGlobalForces::CUDAGlobalForces(int device_id):
  m_device_id(device_id), m_step(0), m_num_atoms(0), m_d_positions(nullptr), m_d_forces(nullptr) {}

CUDAGlobalForces::~CUDAGlobalForces() {
  if (m_d_positions != nullptr) {
    checkCudaErrors(cudaFree(m_d_positions));
  }
  if (m_d_forces != nullptr) {
    checkCudaErrors(cudaFree(m_d_forces));
  }
}
...
void CUDAGlobalForces::requestAtoms(int numAtoms) {
  ...
  checkCudaErrors(cudaMalloc(&m_d_forces, m_num_atoms * sizeof(double3)));
  checkCudaErrors(cudaMemset(m_d_forces, 0, m_num_atoms * sizeof(double3)));
}
...
bool PytorchForces::applyForce(double* f, size_t force_size) {
  ...
  if (auto apply_force = m_module.find_method("apply_force")) {
    ...
    const auto value_out = (apply_force.value())(input_args).toTensor();
    cudaMemcpy(m_d_forces, value_out.const_data_ptr(), m_num_atoms * sizeof(double3), cudaMemcpyDeviceToDevice);
  } else {
