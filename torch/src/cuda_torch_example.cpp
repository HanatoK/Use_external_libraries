...
int main(int argc, char* argv[]) {
  ...
  double* d_cv_force;
  const double cv_force = 100.0;
  checkCudaErrors(cudaMalloc(&d_cv_force, 1 * sizeof(double)));
  checkCudaErrors(cudaMemcpy(d_cv_force, &cv_force, 1 * sizeof(double), cudaMemcpyHostToDevice));
  ...
  while (std::getline(ifs, line)) {
    ...
    pytorch_forces.applyForce(d_cv_force, 1);
  }
  cudaFree(d_cv_force);
  return 0;
}
